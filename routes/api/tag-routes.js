...
router.get('/:id', async (req, res) => {
  ...
  // be sure to include its associated Product data
  try {
    const tagData = await Tag.findByPk(req.params.id, { include: [{ model: Product, through: ProductTag, as: 'tag_products' }] });

    if (!tagData) {
      res.status(404).json({ errorMessage: `No tag found with id: ${req.params.id}` });
      return;
    }
